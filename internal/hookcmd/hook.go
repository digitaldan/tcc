// Package hookcmd implements `ctmux _hook`, the command Claude Code invokes
...
func Run() {
	tabID := os.Getenv("CTMUX_TAB_ID")
	if tabID == "" {
		return // claude run outside ctmux with our settings file; ignore
	}
